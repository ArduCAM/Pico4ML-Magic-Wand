...
void RasterizeStroke(const int8_t *stroke_points, int stroke_points_count, float x_range,
  ...
  for (int point_index = 0; point_index < (stroke_points_count - 1); ++point_index) {
    // Iterate through the stroke and select two sequential start and end coordinate
    // pairs
    //  to form the a line segment
    // example: Stroke points = [a,b,c...] then we would iterate start=a end=b, start=b
    // end=c...
